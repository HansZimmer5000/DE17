...
public class NetDrawImage extends JComponent implements MouseListener, MouseMotionListener {
    ...
    public NetDrawImage(BufferedWriter bwriter, int width, int height, JComboBox lineStyle, JComboBox lineColor, JComboBox lineThickness) {
    ...
    public void mouseDragged(MouseEvent e) {
        if (enabled) {
            ...
            if (style.equals("Freehand")) {
                ...
                paintComponent(this.getGraphics());
                this.drawClientBox(lastX, lastY, newX-lastX, newY-lastY);
            }
            else if (style.equals("Oval")) {
                paintComponent(this.getGraphics());
                this.drawClientOval((lastX+newX)/2 - (newX-lastX), (lastY+newY)/2 - (newY-lastY), newX-lastX, newY-lastY);
            }
    ...
    public void drawBufferedBox(int x1, int y1, int width, int height, Color color, Stroke stroke) {
    ...
    public void drawBufferedOval(int x1, int y1, int width, int height, Color color, Stroke stroke) {
        Graphics2D componentGraphics = (Graphics2D)this.getGraphics();
        componentGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
        imageGraphics.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
        componentGraphics.setStroke(stroke);
    ...
    public void drawClientBox(int x1, int y1, int width, int height) {
        Graphics2D componentGraphics = (Graphics2D)this.getGraphics();
        componentGraphics.setColor(currentColor);
        componentGraphics.setStroke(currentStroke);
            componentGraphics.drawRect(x1, y1, width, height);
    ...
    public void drawClientOval(int x1, int y1, int width, int height) {
        Graphics2D componentGraphics = (Graphics2D)this.getGraphics();
        componentGraphics.setColor(currentColor);
        componentGraphics.setStroke(currentStroke);
            componentGraphics.drawOval(x1, y1, width, height);
    ...
    public void mouseReleased(MouseEvent e) {
        if (enabled) {
            ...
            if (style.equals("Freehand")) {
                ...
                this.drawBufferedBox(lastX, lastY, newX-lastX, newY-lastY, currentColor, currentStroke);
                try {
                    sendLine("box " + lastX + " " + lastY + " " + (newX-lastX) + " " + (newY-lastY) + " " + currentColor.getRGB() + " " + currentStroke.getLineWidth());
                }
                catch (IOException ie) {
                    this.setEnabled(false);
                }
                ...
                this.drawBufferedOval((lastX+newX)/2 - (newX-lastX), (lastY+newY)/2 - (newY-lastY), newX-lastX, newY-lastY, currentColor, currentStroke);
                try {
                    sendLine("oval " + ((lastX+newX)/2 - (newX-lastX)) + " " + ((lastY+newY)/2 - (newY-lastY)) + " " + (newX-lastX) + " " + (newY-lastY) + " " + currentColor.getRGB() + " " + currentStroke.getLineWidth());
                }
                catch (IOException ie) {
                    this.setEnabled(false);
                }
                ...
                this.drawBufferedBox(lastX, lastY, newX-lastX, newY-lastY, Color.white, currentStroke);
                try {
